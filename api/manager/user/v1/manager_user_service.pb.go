// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.34.1
// 	protoc        v4.24.4
// source: api/manager/user/manager_user_service.proto

package v1

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	emptypb "google.golang.org/protobuf/types/known/emptypb"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_api_manager_user_manager_user_service_proto protoreflect.FileDescriptor

var file_api_manager_user_manager_user_service_proto_rawDesc = []byte{
	,
}

var file_api_manager_user_manager_user_service_proto_goTypes = []interface{}{
	(*GetUserRequest)(nil),                   // 0: manager.api.manager.user.v1.GetUserRequest
	(*ListUserRequest)(nil),                  // 1: manager.api.manager.user.v1.ListUserRequest
	(*CreateUserRequest)(nil),                // 2: manager.api.manager.user.v1.CreateUserRequest
	(*UpdateUserRequest)(nil),                // 3: manager.api.manager.user.v1.UpdateUserRequest
	(*UpdateUserStatusRequest)(nil),          // 4: manager.api.manager.user.v1.UpdateUserStatusRequest
	(*DeleteUserRequest)(nil),                // 5: manager.api.manager.user.v1.DeleteUserRequest
	(*emptypb.Empty)(nil),                    // 6: google.protobuf.Empty
	(*ResetUserPasswordRequest)(nil),         // 7: manager.api.manager.user.v1.ResetUserPasswordRequest
	(*UpdateCurrentUserRequest)(nil),         // 8: manager.api.manager.user.v1.UpdateCurrentUserRequest
	(*UpdateCurrentUserRoleRequest)(nil),     // 9: manager.api.manager.user.v1.UpdateCurrentUserRoleRequest
	(*UpdateCurrentUserPasswordRequest)(nil), // 10: manager.api.manager.user.v1.UpdateCurrentUserPasswordRequest
	(*UpdateCurrentUserSettingRequest)(nil),  // 11: manager.api.manager.user.v1.UpdateCurrentUserSettingRequest
	(*SendCurrentUserCaptchaRequest)(nil),    // 12: manager.api.manager.user.v1.SendCurrentUserCaptchaRequest
	(*UserLoginRequest)(nil),                 // 13: manager.api.manager.user.v1.UserLoginRequest
	(*ListLoginLogRequest)(nil),              // 14: manager.api.manager.user.v1.ListLoginLogRequest
	(*GetUserReply)(nil),                     // 15: manager.api.manager.user.v1.GetUserReply
	(*ListUserReply)(nil),                    // 16: manager.api.manager.user.v1.ListUserReply
	(*CreateUserReply)(nil),                  // 17: manager.api.manager.user.v1.CreateUserReply
	(*UpdateUserReply)(nil),                  // 18: manager.api.manager.user.v1.UpdateUserReply
	(*UpdateUserStatusReply)(nil),            // 19: manager.api.manager.user.v1.UpdateUserStatusReply
	(*DeleteUserReply)(nil),                  // 20: manager.api.manager.user.v1.DeleteUserReply
	(*ResetUserPasswordReply)(nil),           // 21: manager.api.manager.user.v1.ResetUserPasswordReply
	(*UpdateCurrentUserReply)(nil),           // 22: manager.api.manager.user.v1.UpdateCurrentUserReply
	(*UpdateCurrentUserRoleReply)(nil),       // 23: manager.api.manager.user.v1.UpdateCurrentUserRoleReply
	(*UpdateCurrentUserPasswordReply)(nil),   // 24: manager.api.manager.user.v1.UpdateCurrentUserPasswordReply
	(*UpdateCurrentUserSettingReply)(nil),    // 25: manager.api.manager.user.v1.UpdateCurrentUserSettingReply
	(*SendCurrentUserCaptchaReply)(nil),      // 26: manager.api.manager.user.v1.SendCurrentUserCaptchaReply
	(*GetUserLoginCaptchaReply)(nil),         // 27: manager.api.manager.user.v1.GetUserLoginCaptchaReply
	(*UserLoginReply)(nil),                   // 28: manager.api.manager.user.v1.UserLoginReply
	(*UserRefreshTokenReply)(nil),            // 29: manager.api.manager.user.v1.UserRefreshTokenReply
	(*ListLoginLogReply)(nil),                // 30: manager.api.manager.user.v1.ListLoginLogReply
}
var file_api_manager_user_manager_user_service_proto_depIdxs = []int32{
	0,  // 0: manager.api.manager.user.v1.User.GetUser:input_type -> manager.api.manager.user.v1.GetUserRequest
	1,  // 1: manager.api.manager.user.v1.User.ListUser:input_type -> manager.api.manager.user.v1.ListUserRequest
	2,  // 2: manager.api.manager.user.v1.User.CreateUser:input_type -> manager.api.manager.user.v1.CreateUserRequest
	3,  // 3: manager.api.manager.user.v1.User.UpdateUser:input_type -> manager.api.manager.user.v1.UpdateUserRequest
	4,  // 4: manager.api.manager.user.v1.User.UpdateUserStatus:input_type -> manager.api.manager.user.v1.UpdateUserStatusRequest
	5,  // 5: manager.api.manager.user.v1.User.DeleteUser:input_type -> manager.api.manager.user.v1.DeleteUserRequest
	6,  // 6: manager.api.manager.user.v1.User.GetCurrentUser:input_type -> google.protobuf.Empty
	7,  // 7: manager.api.manager.user.v1.User.ResetUserPassword:input_type -> manager.api.manager.user.v1.ResetUserPasswordRequest
	8,  // 8: manager.api.manager.user.v1.User.UpdateCurrentUser:input_type -> manager.api.manager.user.v1.UpdateCurrentUserRequest
	9,  // 9: manager.api.manager.user.v1.User.UpdateCurrentUserRole:input_type -> manager.api.manager.user.v1.UpdateCurrentUserRoleRequest
	10, // 10: manager.api.manager.user.v1.User.UpdateCurrentUserPassword:input_type -> manager.api.manager.user.v1.UpdateCurrentUserPasswordRequest
	11, // 11: manager.api.manager.user.v1.User.UpdateCurrentUserSetting:input_type -> manager.api.manager.user.v1.UpdateCurrentUserSettingRequest
	12, // 12: manager.api.manager.user.v1.User.SendCurrentUserCaptcha:input_type -> manager.api.manager.user.v1.SendCurrentUserCaptchaRequest
	6,  // 13: manager.api.manager.user.v1.User.GetUserLoginCaptcha:input_type -> google.protobuf.Empty
	13, // 14: manager.api.manager.user.v1.User.UserLogin:input_type -> manager.api.manager.user.v1.UserLoginRequest
	6,  // 15: manager.api.manager.user.v1.User.UserLogout:input_type -> google.protobuf.Empty
	6,  // 16: manager.api.manager.user.v1.User.UserRefreshToken:input_type -> google.protobuf.Empty
	14, // 17: manager.api.manager.user.v1.User.ListLoginLog:input_type -> manager.api.manager.user.v1.ListLoginLogRequest
	15, // 18: manager.api.manager.user.v1.User.GetUser:output_type -> manager.api.manager.user.v1.GetUserReply
	16, // 19: manager.api.manager.user.v1.User.ListUser:output_type -> manager.api.manager.user.v1.ListUserReply
	17, // 20: manager.api.manager.user.v1.User.CreateUser:output_type -> manager.api.manager.user.v1.CreateUserReply
	18, // 21: manager.api.manager.user.v1.User.UpdateUser:output_type -> manager.api.manager.user.v1.UpdateUserReply
	19, // 22: manager.api.manager.user.v1.User.UpdateUserStatus:output_type -> manager.api.manager.user.v1.UpdateUserStatusReply
	20, // 23: manager.api.manager.user.v1.User.DeleteUser:output_type -> manager.api.manager.user.v1.DeleteUserReply
	15, // 24: manager.api.manager.user.v1.User.GetCurrentUser:output_type -> manager.api.manager.user.v1.GetUserReply
	21, // 25: manager.api.manager.user.v1.User.ResetUserPassword:output_type -> manager.api.manager.user.v1.ResetUserPasswordReply
	22, // 26: manager.api.manager.user.v1.User.UpdateCurrentUser:output_type -> manager.api.manager.user.v1.UpdateCurrentUserReply
	23, // 27: manager.api.manager.user.v1.User.UpdateCurrentUserRole:output_type -> manager.api.manager.user.v1.UpdateCurrentUserRoleReply
	24, // 28: manager.api.manager.user.v1.User.UpdateCurrentUserPassword:output_type -> manager.api.manager.user.v1.UpdateCurrentUserPasswordReply
	25, // 29: manager.api.manager.user.v1.User.UpdateCurrentUserSetting:output_type -> manager.api.manager.user.v1.UpdateCurrentUserSettingReply
	26, // 30: manager.api.manager.user.v1.User.SendCurrentUserCaptcha:output_type -> manager.api.manager.user.v1.SendCurrentUserCaptchaReply
	27, // 31: manager.api.manager.user.v1.User.GetUserLoginCaptcha:output_type -> manager.api.manager.user.v1.GetUserLoginCaptchaReply
	28, // 32: manager.api.manager.user.v1.User.UserLogin:output_type -> manager.api.manager.user.v1.UserLoginReply
	6,  // 33: manager.api.manager.user.v1.User.UserLogout:output_type -> google.protobuf.Empty
	29, // 34: manager.api.manager.user.v1.User.UserRefreshToken:output_type -> manager.api.manager.user.v1.UserRefreshTokenReply
	30, // 35: manager.api.manager.user.v1.User.ListLoginLog:output_type -> manager.api.manager.user.v1.ListLoginLogReply
	18, // [18:36] is the sub-list for method output_type
	0,  // [0:18] is the sub-list for method input_type
	0,  // [0:0] is the sub-list for extension type_name
	0,  // [0:0] is the sub-list for extension extendee
	0,  // [0:0] is the sub-list for field type_name
}

func init() { file_api_manager_user_manager_user_service_proto_init() }
func file_api_manager_user_manager_user_service_proto_init() {
	if File_api_manager_user_manager_user_service_proto != nil {
		return
	}
	file_api_manager_user_manager_user_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_manager_user_manager_user_service_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_manager_user_manager_user_service_proto_goTypes,
		DependencyIndexes: file_api_manager_user_manager_user_service_proto_depIdxs,
	}.Build()
	File_api_manager_user_manager_user_service_proto = out.File
	file_api_manager_user_manager_user_service_proto_rawDesc = nil
	file_api_manager_user_manager_user_service_proto_goTypes = nil
	file_api_manager_user_manager_user_service_proto_depIdxs = nil
}
