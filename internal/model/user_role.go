...
func (ur *UserRole) OneByUserAndRole(ctx kratos.Context, userId, roleId uint32) error {
	return ctx.DB().First(ur, "user_id=? and role_id=?", userId, roleId).Error
}

func (ur *UserRole) Add(ctx kratos.Context) error {
	return ctx.DB().Create(&ur).Error
}

// Update 批量更新角色所属菜单
func (ur *UserRole) Update(ctx kratos.Context, userId uint32, roleIds []uint32) error {
	// 组装新的菜单数据
...
func (ur *UserRole) UserRoles(ctx kratos.Context, userId uint32) ([]*UserRole, error) {
	var list []*UserRole
	return list, ctx.DB().Preload("Role").Find(&list, "user_id=?", userId).Error
}

// DeleteByID 通过id删除
func (ur *UserRole) DeleteByID(ctx kratos.Context, id uint32) error {
