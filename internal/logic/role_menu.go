...
func (r *RoleMenu) CurrenMenuTree(ctx kratos.Context) (*v1.CurrentRoleMenuTreeReply, error) {
	menu := model.Menu{}
...
func (r *RoleMenu) GetMenuIds(ctx kratos.Context, in *v1.GetRoleMenuIdsRequest) (*v1.GetRoleMenuIdsReply, error) {
	// 判断是否具有此角色的权限
	user := model.User{}
...
func (r *RoleMenu) Update(ctx kratos.Context, in *v1.UpdateRoleMenuRequest) (*emptypb.Empty, error) {
	...
	if md.RoleId(ctx) != consts.SuperAdmin {
		for _, id := range in.MenuIds {
			if !util.InList(reply.List, id) {
				return nil, v1.ErrorMenuPermissionsFormat(fmt.Sprintf("menu_id=%d", id))
			}
		}
	}
